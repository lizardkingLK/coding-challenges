using System.Text;
using wcTool.Core.Helpers;
using wcTool.Core.Shared.Values;
...
public class FileBasedWordCount
{
    ...
    private static void HandleResults(
        IEnumerable<Result<(List<long?>, string)>> results,
        HashSet<Delegate> actions)
    {
        if (!results.Any())
        {
        ...
        if (hasMultiple)
        {
            ...
            foreach (Result<(List<long?>, string)> result in results)
            {
                ...
                aggregate = [.. aggregate.Select((item, index) => item + currentPathValues[index] ?? 0)];
            }
    ...
    private static Result<List<long?>> HandleResponseForPath(
        ...
        HashSet<Delegate> actions)
    {
        Result<string> processedPathResult = PathHelper.GetProcessedFilePath(path);
        ...
        Dictionary<string, long?> resultMap = [];
        ...
        return new Result<List<long?>>([.. resultMap.Values], null);
    }
        ...
        List<Result<List<string>>> errors)
    {
        ...
        arguments = [.. arguments.Except(inputs)];
        ...
        paths.AddRange([.. arguments.Except(inputs)]);
